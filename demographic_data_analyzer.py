...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = percentage_bachelors = ((df['education'] == 'Bachelors').sum() / len(df)) * 100
    ...
    higher_education = df[df['education'].isin(high_education)]
    lower_education = df[~df['education'].isin(high_education)]
    ...
    rich_percentage = len(df[(df['hours-per-week'] == min_hours) & (df['salary'] == '>50K')]) / num_min_workers * 100

    # What country has the highest percentage of people that earn >50K?
    rich_people = df[df['salary'] == '>50K']
    rich_by_country = rich_people['native-country'].value_counts()
    total_by_country = df['native-country'].value_counts()
    rich_percentage = (rich_by_country / total_by_country) * 100
